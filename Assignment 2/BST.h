...
struct BST {
		...
		//(b)
		bool deepestNodes();
		void deepest(BTNode*, int, int);
		void findDeepestNode(BTNode*, int, int&);

		//(c)
		bool display(int, int);
		void inOrderPrint(BTNode*);
		void inOrderPrintToFile(BTNode*, ofstream&);
		void reverseInOrderTraversal(BTNode*);
		void reverseInOrderTraversalToFile(BTNode*, ofstream&);

		//(d)
		bool cloneSubtree(BST, type);
		BTNode* findNode(BTNode*, type) const;
		BTNode* cloneNodeRecursive(BTNode*);
		...
		//(f)
		bool printpath();
		void printPathHelper(BTNode*, vector<int>& );
		...
		bool duplicate(BTNode*, const Student&);
};
#endif